...
namespace Octrees
{
    public class OctreeNode
    {
        ...
        public OctreeNode(Bounds bounds, float minNodeSize)
        {
            ...
            Vector3 centerOffset = bounds.size * .25f; // quater size
            Vector3 parentCenter = bounds.center;

            for (int i=0; i<8; i++)
            {
                Vector3 childerCenter = parentCenter;
                childerCenter.x += centerOffset.x * ((i & 1) == 0 ? -1 : 1);
                childerCenter.y += centerOffset.y * ((i & 2) == 0 ? -1 : 1);
                childerCenter.z += centerOffset.z * ((i & 4) == 0 ? -1 : 1);
                childBounds[i] = new Bounds(childerCenter, newSize);
            }
        }
